...
PROJ_ROOT = osp.normpath("/mnt/data/work/synthetic-data-yolo-training_and_pose_estimation/src") # added the path to src this time
...
DATASETS_ROOT = osp.normpath(osp.join(PROJ_ROOT, "output"))
...
class MY_DATASET_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "bop/train_pbr"),
        )
...
def get_my_dataset_metadata(obj_names, ref_key):
...
for obj in ref.mydataset.objects:
    for split in ["train_pbr"]:
        ...
        if name not in SPLITS_MY_DATASET_PBR:
            SPLITS_MY_DATASET_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MY_DATASET_PBR:
        used_cfg = SPLITS_MY_DATASET_PBR[name]
    else:
...
def get_available_datasets():
    return list(   SPLITS_MY_DATASET_PBR.keys())
